...
[ "workers" ].each { |lib_subdir|
  ...
  rel_path = File.join(Rails.root.to_s,"vendor","plugins","talho","lib",lib_subdir)
...
# Create links in Rails.root.to_s/public so that the register_javascript_expansion()
# and register_stylesheet_expansion() methods can see the plugin's files.
parent_public_dir = File.join(Rails.root.to_s, "public")
[ "javascripts", "stylesheets", "training" ].each { |public_subdir|
  rel_path = File.join("..","..","vendor","plugins","talho","public",public_subdir)
  target = File.join(parent_public_dir, public_subdir, "talho")
  Dir.ensure_exists(File.join(parent_public_dir, public_subdir))
  File.symlink(rel_path, target) unless File.symlink?(target)
}
